...
var GitAction = class {
  ...
  async clone(url, path3) {
    try {
      await this.git.clone(url, path3);
    } catch (error) {
      throw new GitOperationError("clone", error.message);
    }
  }
  async checkoutBranch(path3, branch) {
    try {
...
var FileScanner = class {
  ...
  // 查找模块文件
  async findModuleFile(importPath, currentDir, basePath) {
  ...
  // [依赖文件按需分析]: 分析依赖文件
  async analyzeDependencies(content, filePath, basePath) {
    const dependencies = [];
  ...
  // 扫描目录
  async scanDirectory(path3, options) {
    if (!path3) {
  ...
  // 扫描目标文件及其依赖文件
  async processFileAndDependencies(basePath, relativePath, options, allFiles) {
  ...
  // 尝试查找文件
  async tryFindFile(basePath, filePath, options) {
    try {
  ...
  // 扫描文件
  async processFile(basePath, relativePath, options) {
    try {
...
var GitIngest = class {
  ...
  // 分析扫描目录
  async analyzeFromDirectory(dirPath, options) {
    if (!dirPath) {
      throw new ValidationError("Path is required");
    }
    if (!existsSync(dirPath)) {
