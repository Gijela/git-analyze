 ...
 */
function analyzeCode(sourceCode: string): CodeAnalysisResult {
  const result: CodeAnalysisResult = {
    dependencies: { imports: [], from: [], exports: [] },
    asyncFlow: { operations: [], errorHandling: [] },
    codeQuality: { complexity: 0 },
    businessLogic: { conditions: [], loops: [], functionCalls: [] },
  };

  const tree = parser.parse(sourceCode);
  analyzeNode(tree.rootNode, result);
 ...
 * @param {CodeAnalysisResult} result - 分析结果
 */
function analyzeNode(node: any, result: CodeAnalysisResult): void {
  // 分析导入关系
  if (node.type === "import_statement") {
    const importRawPath = node.text;
    const importMatch = importRawPath.match(/import\s+(?:{([^}]+)}|([^{}\s]+))/);
    const importContent = importMatch ? (importMatch[1] || importMatch[2]) : '';

    const fromMatch = importRawPath.match(/from\s+['"]([^'"]+)['"]/);
    let fromContent = '';
    if (fromMatch) {
      const importMatch = fromMatch[1].match(/(.+?)\s*import/);
      fromContent = importMatch ? importMatch[1].trim() : fromMatch[1].trim();
    }

    result.dependencies.imports.push(importContent.trim());
    result.dependencies.from.push(fromContent.trim());
  }

  // 分析导出
  else if (node.type === "export_statement") {
    const rawTotalCode = node.text;
    const exportMatch = rawTotalCode.match(/export\s+(.+)/);
    const exportContent = exportMatch ? (exportMatch[1] || exportMatch[2]) : '';
    result.dependencies.exports.push(exportContent.trim());
  }

  // 分析异步流程
  else if (node.type === "await_expression" || isAsyncFunction(node) || node.type === "arrow_function") {
    result.asyncFlow.operations.push(node.child(1)?.text || '');
  }

  // 分析错误处理
  else if (node.type === "try_statement") {
    result.asyncFlow.errorHandling.push(node.child(2)?.text || '');
  }

  // 分析代码质量, 计算圈复杂度
  else if (node.type === "function_declaration" || node.type === "method_definition" || node.type === "arrow_function") {
    let complexity = 1;
    traverseNode(node, (n: any) => {
      if (["if_statement", "for_statement", "while_statement", "catch_clause", "arrow_function", "switch_statement"].includes(n.type)) {
        complexity++;
      }
    });
    result.codeQuality.complexity += complexity;
  }

  // 分析业务逻辑 - 条件语句
  else if (node.type === "if_statement") {
    result.businessLogic.conditions.push(node.child(1)?.text || node.text);
  }

  // 分析业务逻辑 - 循环语句
  else if (node.type === "for_statement" || node.type === "while_statement") {
    const loop = node.text;
    result.businessLogic.loops.push(loop);
  }

  // 分析业务逻辑 - 函数调用
  else if (node.type === "call_expression") {
    const functionName = node.child(0)?.text || node.text;
    result.businessLogic.functionCalls.push(functionName);
  }
...
function isAsyncFunction(node: any): boolean {
  return node.type === "function_declaration" && node.children.some((c: any) => c.type === "async");
}

// 使用示例
function generateAnalysisReport(sourceCode: string): void {
  const analysis = analyzeCode(sourceCode);
  console.log("代码分析报告：", JSON.stringify(analysis, null, 2));
}
...
generateAnalysisReport(codeAsString);